...
#  数据持久化：后端连接 MongoDB Atlas，服务重启/休眠后数据不会丢失
#
# ============================================================
...
5. 添加环境变量：
   ...
   - Key 填：`MONGODB_URI`
   - Value 填：你的 MongoDB Atlas 连接字符串
     （格式如：`mongodb+srv://用户名:密码@cluster0.xxx.mongodb.net/?retryWrites=true&w=majority`）
...
- **数据持久性**：位置数据存储在 MongoDB Atlas 云数据库中
  Render 服务重启或休眠后，设备和历史位置数据都不会丢失
